...
//Array of items
var items = []; //Render items in the codeblock

function renderItem(item) {
  var list = document.querySelector('#itemList'); //Create an 'li' element

  var node = document.createElement("li"); //Set ID

  node.setAttribute('data-key', item.id);
  node.innerHTML = "".concat(item.text);
  list.append(node);
...
form.addEventListener('submit', function (event) {
  ...
  event.preventDefault(); //select text input

  var itemName = document.getElementById("itemName").value; //Capitalize item title
  ...
  var titleBlock = "&lt;h3&gt; &lt;a href=\"".concat(itemLink, "\"&gt;").concat(itemName, "&lt;/a&gt;, $").concat(itemPrice, "&lt;/h3&gt;<br><br>").concat(itemDescription);
  var text = titleBlock;
